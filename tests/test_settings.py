...
#: Settings mechanics are exercised as the operator; permission behaviour has
#: its own test module (tests/test_permissions.py).
...
def test_config_seed_is_used_when_no_row_exists(settings_store):
    assert not store.has_row("models.chat", OPERATOR)
...
def test_a_row_takes_precedence_over_config(settings_store):
    ...
    store.set("models.chat", "some-other-model", OPERATOR)

    assert store.get("models.chat", OPERATOR) == "some-other-model"
    assert store.get("models.chat", OPERATOR) != seed
...
def test_the_existing_config_accessor_is_the_settings_first_read_path(settings_store):
    ...
    store.set("models.chat", "swapped-model", OPERATOR)
...
def test_model_options_resolves_each_key_settings_first(settings_store):
    ...
    store.set("model_options.temperature", 0.9, OPERATOR)
...
def test_clearing_a_row_falls_back_to_the_config_seed_again(settings_store):
    seed = config.get("models", "chat")
    store.set("models.chat", "temporary", OPERATOR)
    assert config.chat_model() == "temporary"

    assert store.clear("models.chat", OPERATOR) is True

    assert config.chat_model() == seed
    assert store.describe("models.chat", OPERATOR).source == "config"
    assert store.clear("models.chat", OPERATOR) is False
...
def test_get_and_section_stay_pure_config_reads(settings_store):
    ...
    store.set("models.chat", "db-value", OPERATOR)
...
def test_a_write_takes_effect_immediately_without_reload(settings_store):
    ...
    store.set("model_options.temperature", 0.11, OPERATOR)
...
def test_the_cache_is_actually_dropped_on_write_not_just_overwritten(settings_store):
    ...
    """
    store.set("models.chat", "first", OPERATOR)
    assert store.get("models.chat", OPERATOR) == "first"
    ...
    store.invalidate()
    assert store.get("models.chat", OPERATOR) == "second"
...
def test_reads_do_not_hit_the_database_once_cached(settings_store, monkeypatch):
    ...
    """
    store.set("models.chat", "cached-model", OPERATOR)
...
def test_the_cache_is_keyed_by_store_path_not_shared_across_stores(
    ...
    db.init_databases()
    store.reset_cache()
    store.set("models.chat", "store-one-model", OPERATOR)
...
def test_values_round_trip_with_their_declared_type(settings_store):
    store.set("model_options.num_ctx", 16384, OPERATOR)
    store.set("model_options.temperature", 0.7, OPERATOR)
    store.set("model_options.think", True, OPERATOR)
    store.set("models.embedding", "some-embedder", OPERATOR)

    assert store.get("model_options.num_ctx", OPERATOR) == 16384
    assert isinstance(store.get("model_options.num_ctx", OPERATOR), int)
    assert store.get("model_options.temperature", OPERATOR) == 0.7
    assert isinstance(store.get("model_options.temperature", OPERATOR), float)
    assert store.get("model_options.think", OPERATOR) is True
    assert store.get("models.embedding", OPERATOR) == "some-embedder"
...
def test_a_wrong_type_is_refused_at_write_time(settings_store):
    ...
    with pytest.raises(store.SettingTypeError, match="declared int"):
        store.set("model_options.num_ctx", "not-a-number", OPERATOR)
    with pytest.raises(store.SettingTypeError, match="declared bool"):
        store.set("model_options.think", "yes", OPERATOR)
    assert not store.has_row("model_options.num_ctx", OPERATOR)
...
def test_a_bool_is_not_accepted_as_an_int(settings_store):
    ...
    with pytest.raises(store.SettingTypeError):
        store.set("model_options.num_ctx", True, OPERATOR)
...
def test_a_corrupt_row_raises_rather_than_silently_using_the_seed(settings_store):
    ...
    store.set("model_options.num_ctx", 8192, OPERATOR)
    with db.transaction() as conn:
        conn.execute(
    ...
    with pytest.raises(store.SettingTypeError, match="not a valid int"):
        store.get("model_options.num_ctx", OPERATOR)


def test_the_stored_value_type_matches_the_registry(settings_store):
    store.set("model_options.temperature", 0.5, OPERATOR)
...
def test_bootstrap_keys_are_not_settable(settings_store):
    ...
    for name in ("paths.data_dir", "api.port", "ollama.host"):
        with pytest.raises(store.UnknownSettingError):
            store.set(name, "anything", OPERATOR)
...
def test_unknown_key_names_what_is_known(settings_store):
    with pytest.raises(store.UnknownSettingError, match="models.chat"):
        store.get("models.nonexistent", OPERATOR)
...
def test_updated_by_and_updated_at_are_recorded(settings_store):
    store.set("models.chat", "attributed", Actor("u-lyle", "Lyle", Role.ADMIN))
...
def test_describe_all_covers_the_whole_registry(settings_store):
    store.set("model_options.temperature", 0.42, OPERATOR)
    described = {d.name: d for d in store.describe_all(OPERATOR)}
